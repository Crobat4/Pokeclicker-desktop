...
function createWindow() {
  ...
  mainWindow.setTitle('PokéClicker - Crobat Fork');
...
function createSecondaryWindow() {
  ...
  newWindow.setTitle('PokéClicker - Crobat Fork (alternate)');
...
if (!isMainInstance) {
  ...
  */
  ...
  const downloadUpdate = async (initial = false) => {
    ...
    https.get('https://codeload.github.com/Crobat4/pokeclicker/zip/master-crobat', async res => {
      ...
      try {
        ...
        if (!initial) await mainWindow.webContents.executeJavaScript(`Notifier.notify({ title: '[UPDATER] Crobat v${newCrobatVersion}', message: 'Downloading Files...<br/><span id="update-message-progress">Please Wait...</span>', timeout: 1e6 })`);
      }catch(e){}
      ...
      res.pipe(file).on('finish', async () => {
        ...
        const userResponse = dialog.showMessageBoxSync(mainWindow, {
          title: 'PokeClicker - Crobat Fork - Update success!',
  ...
  const downloadUpdateFailed = () => {
    ...
    const userResponse = dialog.showMessageBoxSync(mainWindow, {
      type: 'error',
      title: 'PokeClicker - Crobat Fork - Update failed!',
    ...
    const request = https.get('https://raw.githubusercontent.com/Crobat4/pokeclicker/master-crobat/package.json', res => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          const newCrobatVersionAvailable = isNewerVersion(data.crobatVersion);
  ...
  try {
    autoUpdater.on('update-downloaded', () => {
      const userResponse = dialog.showMessageBoxSync(mainWindow, {
        title: 'PokeClicker - Crobat Fork - Client Update Available!',
